...
function tanggal_waktu($t){
	...
	$pecah_t=explode(' ',$t);
	$calendar=$pecah_t[0];
	$pecah_calendar=explode('-',$calendar);
	$tanggal=$pecah_calendar[2];
	$index=date('F',strtotime($calendar));
	$bulan=$month[$index];
	$tahun=$pecah_calendar[0];
	
	return $tanggal.' '.$bulan.' '.$tahun.' '.$pecah_t[1];
}
//FORMAT RUPIAH
function rupiah($rp)
{
				...
				<fieldset> 
					<table>
						...
						<tr>
							...
							<td><input type="datetime-local" name="tanggal1"></td>
						</tr>
						
						<tr>
							<td  style="width:40%">Sampai Tanggal :</b></td>	
							
							<td><input type="datetime-local" name="tanggal2"></td>
						</tr>
